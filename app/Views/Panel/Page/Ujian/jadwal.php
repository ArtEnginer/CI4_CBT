                    ...
                    <table class="table app-table-hover mb-0 text-left datatables-init">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="cell">
                                    <?php if ($waktu->isAfter($item->waktu) && $waktu->isBefore($item->waktu->addMinutes($item->tenggat))) : ?>
                                    <a class="btn app-btn-primary"
                                        href="<?= route_to('ujian-masuk', $item->token_ujian) ?>">Masuk</a>
                                    <?php elseif ($waktu->isAfter($item->waktu->addMinutes($item->tenggat))) : ?>
                                    <a class="btn app-btn-secondary"
                                        href="<?= route_to('ujian-nilai', $item->token_ujian) ?>">Lihat Nilai</a>
                                    <?php else : ?>
